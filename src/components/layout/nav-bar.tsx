...
export default function Navbar() {
    ...
    return (
        <header
            className={cn(
                "sticky top-0 z-50 h-[88px] border-b border-border bg-foreground backdrop-blur-md transition-all duration-300",
                scrolled && "shadow-sm"
            )}
        >
            <div className="mx-auto flex h-full max-w-7xl items-center justify-between gap-6 px-4 md:px-6 lg:px-8">
                ...
                <nav className="hidden items-center gap-1 lg:flex">
                    ...
                    {NAV_LINKS.map((link) => (
                        <Link
                            key={link.label}
                            href={link.href}
                            className="rounded-full px-4 py-2 text-sm font-medium  transition-all duration-200 hover:bg-[#E96D8E]/10 text-muted hover:text-[#E96D8E]"
                        >
                            {link.label}
                        </Link>
                    ))}
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        ...
                        {cartCount > 0 && (
                            <span className="absolute right-1 top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[#E96D8E] text-[10px] font-bold text-white">
                                {cartCount}
                            </span>
                        )}
                    </button>
                    <Link
                        href="#booking"
                        className="ml-2 hidden rounded-full bg-[#305D3D] px-6 py-3 text-sm font-medium text-white transition-all duration-300 hover:scale-105 hover:bg-[#264B31] sm:inline-flex"
                    >
                        Shop Now
                    </Link>
                    <button
            ...
            {menuOpen && (
                <div className="border-t border-border bg-[#FFFCF8] px-5 py-5 shadow-lg lg:hidden">
                    <nav className="flex flex-col gap-1">
                        ...
                        <Link
                            href="#booking"
                            className="mt-3 flex justify-center rounded-full bg-[#305D3D] px-6 py-3 text-sm font-medium text-white transition-all duration-300 hover:bg-[#264B31]"
                        >
                            Shop Now
                        </Link>
